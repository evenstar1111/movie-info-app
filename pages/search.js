...
export default class Search extends React.Component {
   ...
   render() {
      const { movies, loading, error, message, kw, type } = this.state.values;
      const { isOpen } = this.state;

      const formDisabled = loading ? true : false;
      const searchBar = (
         <Container className={`${styles.srch_wpr} py-2`} fluid>
            <div id="search_page_collapse" className={`${styles._collapse} collapse ${isOpen ? 'show' : 'hide'}`}>
      ...
      const errorComp = error && (
         <div className="alert alert-danger" role="alert">
            {message}
            <button type="button" className="close mb-3" aria-label="Close" onClick={this.closeErrorMsg}>
            ...
            {!loading && (
               <Container className="mt-2" fluid>
                  <Row className="justify-content-center" noGutters>
                     {movies ? movies.results && <MovieCard movies={movies.results} type={type} /> : ''}
                  </Row>
               </Container>
            )}
         </Layout>
